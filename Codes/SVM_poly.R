...
#successful rate is 0.735, it doesn't change when cost change
for (j in select_value){
  svm.poly_train1 = svm(Attrition~., data=train1, kernel="polynomial", cost=j)
  svm.poly_train2 = svm(Attrition~., data=train2, kernel="polynomial", cost=j)
  svm.poly_train3 = svm(Attrition~., data=train3, kernel="polynomial", cost=j)
  svm.poly_train4 = svm(Attrition~., data=train4, kernel="polynomial", cost=j)
  svm.poly_train5 = svm(Attrition~., data=train5, kernel="polynomial", cost=j)
  result_train1=predict(svm.poly_train1, tun.set)
  result_train2=predict(svm.poly_train2, tun.set)
  result_train3=predict(svm.poly_train3, tun.set)
  result_train4=predict(svm.poly_train4, tun.set)
  result_train5=predict(svm.poly_train5, tun.set)
  final_result=0
  ...
  print(final_result)
  svm.poly_Confusion_matrix=table(final_result,tun.set$Attrition)
  svm.poly_Confusion_matrix
...
#print(f1_vector)
select_value[10]
plot(select_value,sucessful_rate_vector[-1],type='l',xlab='cost',ylab='accuracy',main='Polynomial  SVM accuracy')

text(select_value[100], sucessful_rate_vector[101],labels='optimal(0.5,0.875)',cex= 0.9,pos=3)
points(select_value[100],sucessful_rate_vector[101], col = "red")
...
#svm.radial = svm(Attrition~., data=traindata, kernel="radial", gamma=100, cost=1)
#table(traindata$Attrition, predict(svm.radial, traindata))
#table(testdata$Attrition, predict(svm.radial, testdata))
...
result_train1=predict(svm.poly_train1, test.set)
result_train2=predict(svm.poly_train2, test.set)
result_train3=predict(svm.poly_train3, test.set)
result_train4=predict(svm.poly_train4, test.set)
result_train5=predict(svm.poly_train5, test.set)
final_result_test=0
...
print(final_result_test)
svm.poly_Confusion_matrix_test=table(final_result_test,test.set$Attrition,dnn = c('Prediction','Truth'))
svm.poly_Confusion_matrix_test
sucessful_rate=sum(diag(svm.poly_Confusion_matrix_test))/270
sucessful_rate

precision=svm.poly_Confusion_matrix_test[2,2]/(svm.poly_Confusion_matrix_test[2,1]+svm.poly_Confusion_matrix_test[2,2])
precision
recall=svm.poly_Confusion_matrix_test[2,2]/(svm.poly_Confusion_matrix_test[1,2]+svm.poly_Confusion_matrix_test[2,2])
recall
f1=2*recall*precision/(precision+recall)
f1
