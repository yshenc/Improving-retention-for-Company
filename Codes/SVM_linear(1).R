...
select_value
#we find optimal value when cost=0.00106. 0.001041 at successful rate 0.88
#linear SVM
for (j in select_value){
  svm.linear_train1 = svm(Attrition~., data=train1, kernel="linear", cost=j)
  svm.linear_train2 = svm(Attrition~., data=train2, kernel="linear", cost=j)
  svm.linear_train3 = svm(Attrition~., data=train3, kernel="linear", cost=j)
  svm.linear_train4 = svm(Attrition~., data=train4, kernel="linear", cost=j)
  svm.linear_train5 = svm(Attrition~., data=train5, kernel="linear", cost=j)
  result_train1=predict(svm.linear_train1, tun.set)
  result_train2=predict(svm.linear_train2, tun.set)
  result_train3=predict(svm.linear_train3, tun.set)
  result_train4=predict(svm.linear_train4, tun.set)
  result_train5=predict(svm.linear_train5, tun.set)
  final_result=0
  ...
  #print(final_result)
  svm.linear_Confusion_matrix=table(final_result,tun.set$Attrition)
  svm.linear_Confusion_matrix
  sucessful_rate=sum(diag(svm.linear_Confusion_matrix))/200
  sucessful_rate
  sucessful_rate_vector=c(sucessful_rate_vector,sucessful_rate)
  #precision=svm.linear_Confusion_matrix[1,1]/(svm.linear_Confusion_matrix[1,1]+svm.linear_Confusion_matrix[1,2])
  #recall=svm.linear_Confusion_matrix[1,1]/(svm.linear_Confusion_matrix[1,1]+svm.linear_Confusion_matrix[2,1])
  #f1=2*recall*precision/(precision+recall)
  #f1_vector=c(f1_vector,f1)
}
print(sucessful_rate_vector)
print(select_value)
plot(select_value,sucessful_rate_vector[-1],type='l',xlab='cost',ylab='accuracy',main='Linear SVM accuracy')
text(select_value[1], sucessful_rate_vector[2],labels='optimal(0.001,0.875)',cex= 0.9,pos=4)
points(select_value[1],sucessful_rate_vector[2], col = "red")
...
result_train1=predict(svm.linear_train1, test.set)
result_train2=predict(svm.linear_train2, test.set)
result_train3=predict(svm.linear_train3, test.set)
result_train4=predict(svm.linear_train4, test.set)
result_train5=predict(svm.linear_train5, test.set)
final_result_test=0
...
svm.linear_Confusion_matrix_test=table(final_result_test,test.set$Attrition, dnn = c('Prediction','Truth'))
svm.linear_Confusion_matrix_test
sucessful_rate=sum(diag(svm.linear_Confusion_matrix_test))/270
sucessful_rate
precision=svm.linear_Confusion_matrix_test[2,2]/(svm.linear_Confusion_matrix_test[2,1]+svm.linear_Confusion_matrix_test[2,2])
precision
recall=svm.linear_Confusion_matrix_test[2,2]/(svm.linear_Confusion_matrix_test[1,2]+svm.linear_Confusion_matrix_test[2,2])
recall
f1=2*recall*precision/(precision+recall)
f1
#f1_vector=c(f1_vector,f1)
